...
import json
import sys
#from AngryBirdsLevelGenerator-master.AngryBirdsGA import XmlHelpers
...
def main():
    ...
    config_file = open(os.path.join(project_root, sys.argv[0]), 'r')
    ...
    for generation in range(number_of_generations):
        ...
        xml.writeXML(population[0], os.path.join(project_root, log_path + "/levels-0-"+ log_base_name  +
    ...
    xml.writeXML(population[0], os.path.join(project_root, log_path + "/levela-0-" + log_base_name +
                                             timestamp.strftime("%y%m%d_%H%M%S") + ".xml"))
